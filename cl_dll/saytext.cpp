...
int CHudSayText :: Draw( float flTime )
{
	...
	flScrollTime = _min( flScrollTime, flTime + m_HUD_saytext_time->value );
		...
		if ( *g_szLineBuffer[i] )
		{
			if ( *g_szLineBuffer[i] == 2 && g_pflNameColors[i] )
			{
				...
				if ( buf )
				{
					...
					strncpy( buf, g_szLineBuffer[i], _min(g_iNameLengths[i], MAX_PLAYER_NAME_LENGTH+32) );
					buf[ _min(g_iNameLengths[i], MAX_PLAYER_NAME_LENGTH+31) ] = 0;
...
void CHudSayText :: SayTextPrint( const char *pszBuf, int iBufSize, int clientIndex )
{
	...
	strncpy( g_szLineBuffer[i], pszBuf, _max(iBufSize , MAX_CHARS_PER_LINE) );
