...
void CBeam::SetObjectCollisionBox( void )
{
	...
	pev->absmin.x = _min( startPos.x, endPos.x );
	pev->absmin.y = _min( startPos.y, endPos.y );
	pev->absmin.z = _min( startPos.z, endPos.z );
	pev->absmax.x = _max( startPos.x, endPos.x );
	pev->absmax.y = _max( startPos.y, endPos.y );
	pev->absmax.z = _max( startPos.z, endPos.z );
}
#endif
