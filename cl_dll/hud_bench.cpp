...
{
	return g_benchSwitchTimes[ _min( Bench_GetStage(), LAST_STAGE ) ];
}
...
int CHudBenchmark::MsgFunc_Bench(const char *pszName, int iSize, void *pbuf)
{
	...
	m_StoredLatency = _min( 1.0, m_StoredLatency );
	m_StoredLatency = _max( 0.0, m_StoredLatency );
...
void CHudBenchmark::Think( void )
{
	...
	if ( Bench_InStage( FIRST_STAGE ) )
	{
		...
		if ( m_fReceiveTime )
		{
			...
			latency = _max( 0.0, latency );
			latency = _min( 1.0, latency );
...
void CHudBenchmark::SetCompositeScore( void )
{
	...
	composite = _min( 100, composite );
	composite = _max( 0, composite );
	...
	if ( Bench_InStage( THIRD_STAGE ) && strstr( modelname, "ppdemodot" ) )
	{
		...
		if ( Bench_Active() && Bench_InStage( THIRD_STAGE ) )
		{
			...
			if ( dt > 0.0 && dt < 1.0 )
			{
				...
				fZAdjust = _min( fBounds, fZAdjust );
				fZAdjust = _max( -fBounds, fZAdjust );
		...
		*/
		...
		if ( ( NUM_BENCH_OBJ - i - 1 ) < gHUD.m_Benchmark.GetObjects() )
		{
			...
			else
			{
				...
				bench[i].curstate.renderamt = _min( 255, bench[i].curstate.renderamt );
